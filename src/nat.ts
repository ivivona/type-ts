...
type MultiplyAcc<A extends Nat, B extends Nat, ACC extends Nat> = {
  0: ACC;
  n: Prev<Cast<B, Positive>> extends infer R
    ? Add<ACC, A> extends infer S
      ? MultiplyAcc<A, Cast<R, Nat>, Cast<S, Nat>>
      : never
    : never;
...
// Euclidean division
type Div<A extends Nat, B extends Positive, Q extends Nat> = Add<
  Q,
  _1
> extends infer S
  ? {
      end: Q;
      loop: Div<A, B, Cast<S, Nat>>;
    }[GreaterThan<Multiply<B, Cast<S, Nat>>, A> extends true ? "end" : "loop"]
  : never;

export type Quotient<A extends Nat, B extends Positive> = Div<A, B, _0>;

export type Remainder<A extends Nat, B extends Positive> = Quotient<
  A,
  B
> extends infer Q
  ? Substract<A, Multiply<B, Cast<Q, Nat>>>
  : never;
